...
DownloadNow.js implements the `saveAs()` FileSaver interface in browsers that do
not natively support it. There is a [DownloadNow.js demo][1] that demonstrates saving
various media types.

DownloadNow.js is the solution to saving files on the client-side, and is perfect for
...
Pass `true` for `disableAutoBOM` if you don't want DownloadNow.js to automatically provide Unicode text encoding hints (see: [byte order mark](https://en.wikipedia.org/wiki/Byte_order_mark)).
...
The `DownloadNow.js` distribution file is compiled with Uglify.js like so:

```bash
uglifyjs DownloadNow.js --mangle --comments /@source/ > FileSaver.min.js
# or simply:
npm run build
```
